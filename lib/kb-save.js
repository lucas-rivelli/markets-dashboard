...
async function saveKnowledgeItem(input) {
  ...
  if (!result.alreadySaved && !isKnowledgeExcludedCategory(record.category)) {
    try {
      const { ingestWikiItem } = require("./wiki-ingest");
      await ingestWikiItem(record, { useLlm: Boolean(process.env.ANTHROPIC_API_KEY) });
    } catch (err) {
      console.warn("Wiki ingest failed:", err.message);
    }
  }

  return { record, result, persistence };
}
