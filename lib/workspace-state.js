...
function normalizeStatusMap(raw, maxKey = 512) {
  const out = {};
  if (!raw || typeof raw !== "object") return out;
  const allowed = new Set(["inbox", "trash"]);
  for (const [key, value] of Object.entries(raw)) {
    const status = String(value || "").trim().toLowerCase();
    if (status === "saw" || status === "to-read" || status === "read") {
...
function cleanHighlight(raw) {
  ...
  if (!text) return null;
  return {
    id: String(raw.id || "").trim().slice(0, 80) || text.slice(0, 80),
    text: text.slice(0, 2000),
    created_at: raw.created_at || new Date().toISOString(),
  };
}

function normalizeTitlesMap(raw, maxKey = 512, maxLen = 300) {
  const out = {};
  if (!raw || typeof raw !== "object") return out;
  for (const [key, value] of Object.entries(raw)) {
    const title = String(value || "").replace(/\s+/g, " ").trim();
    if (title) out[String(key).slice(0, maxKey)] = title.slice(0, maxLen);
  }
  return out;
}

function normalizeAddedMap(raw, maxKey = 512) {
  const out = {};
  if (!raw || typeof raw !== "object") return out;
  for (const [key, value] of Object.entries(raw)) {
    const stamp = String(value || "").trim();
    if (!stamp || Number.isNaN(Date.parse(stamp))) continue;
    out[String(key).slice(0, maxKey)] = stamp;
  }
  return out;
}

function normalizeHighlightsMap(raw, maxKey = 512) {
  const out = {};
  if (!raw || typeof raw !== "object") return out;
  for (const [key, value] of Object.entries(raw)) {
    if (!Array.isArray(value)) continue;
    const highlights = value.map(cleanHighlight).filter(Boolean);
    if (highlights.length) out[String(key).slice(0, maxKey)] = highlights.slice(0, 200);
  }
  return out;
}
...
function normalizeWorkspace(raw) {
  ...
  return {
    ...
    item_folders: normalizeStringMap(raw.item_folders, 512, 160),
    tags,
    item_tags: normalizeStringMap(raw.item_tags),
    item_status: normalizeStatusMap(raw.item_status),
    item_titles: normalizeTitlesMap(raw.item_titles),
    item_added: normalizeAddedMap(raw.item_added),
    item_highlights: normalizeHighlightsMap(raw.item_highlights),
...
function mergeWorkspace(base, incoming) {
  ...
  const itemTitles = { ...left.item_titles, ...right.item_titles };
  const itemAdded = { ...left.item_added, ...right.item_added };
  const itemHighlights = { ...left.item_highlights };
  for (const [key, highlights] of Object.entries(right.item_highlights)) {
    itemHighlights[key] = highlights;
  }
  ...
  return {
    ...
    item_titles: hasIncoming("item_titles") ? right.item_titles : itemTitles,
    item_added: hasIncoming("item_added") ? right.item_added : itemAdded,
    item_highlights: hasIncoming("item_highlights") ? right.item_highlights : itemHighlights,
...
async function saveWorkspaceState(nextState, { merge = true } = {}) {
  ...
  if (merge) {
    ...
    const hasCurrent =
      ...
      Object.keys(current.item_titles).length ||
      Object.keys(current.item_added).length ||
      Object.keys(current.item_highlights).length ||
      current.read.length ||
      current.saved.length;
    // Merge from the RAW payload: normalizing first fills in every key with
    // empty defaults, which makes hasIncoming() treat omitted domains as
    // client-provided and wipe them server-side.
    if (hasCurrent) state = mergeWorkspace(current, nextState);
  }
