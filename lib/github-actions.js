...
function runCreatedAtMs(run) {
  const raw = run?.created_at || run?.run_started_at;
  if (!raw) return 0;
  const ms = Date.parse(raw);
  return Number.isFinite(ms) ? ms : 0;
}

/** Most recent run created within the last `minIntervalMs` (any status). */
async function findRecentWorkflowRun(workflowFile, minIntervalMs) {
  if (!minIntervalMs || minIntervalMs <= 0) return null;
  const runs = await listRecentWorkflowRuns(workflowFile, { perPage: 10 });
  const cutoff = Date.now() - minIntervalMs;
  return runs.find((run) => runCreatedAtMs(run) >= cutoff) || null;
}

async function dispatchWorkflow(
  workflowFile,
  { ref, skipIfActive = false, minIntervalMs = 0 } = {}
) {
  const config = githubConfig();
  const token = dispatchToken();
  ...
  if (minIntervalMs > 0) {
    const recent = await findRecentWorkflowRun(workflowFile, minIntervalMs);
    if (recent) {
      return {
        dispatched: false,
        skipped: true,
        ...
        workflow: workflowFile,
        run_id: recent.id,
        run_status: recent.status,
        html_url: recent.html_url || null,
