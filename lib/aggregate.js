...
const { loadBriefings } = require("./briefings");
...
async function buildFeedResponse(sources, { force = false } = {}) {
  ...
  const [rssResults, spotifyResult, vicResult, bookmarks, manualResult, writingsResult, briefingsResult] =
    await Promise.all([
    ...
    loadBriefings().then((items) => ({ items })).catch((err) => ({
      items: [],
      error: err.message,
    })),
  ]);
  ...
  const briefings = briefingsResult.items || [];
  if (briefingsResult.error) failed.push("Lucas Briefing");

  allItems.push(...bookmarks);
  allItems.push(...manualLinks);
  allItems.push(...writings);
  allItems.push(...briefings);
