...
async function buildFeedResponse(sources, { force = false } = {}) {
  ...
  const [rssResults, spotifyResult, vicResult, fundLettersResult, bookmarks, manualResult, writingsResult] =
    await Promise.all([
    Promise.allSettled(feedSources.map(fetchFeed)),
    fetchSpotifyEpisodes({ force }).catch((err) => ({
      items: [],
      error: err.message,
    })),
    fetchVicIdeas().catch((err) => ({
      items: [],
      error: err.message,
    })),
    fetchFundLetters().catch((err) => ({
      items: [],
      error: err.message,
      errors: [],
    })),
    ...
    loadWritings().then((items) => ({ items })).catch((err) => ({
      items: [],
      error: err.message,
    })),
  ]);
  ...
  const manualLinkSet = new Set(manualLinks.map((item) => item.link).filter(Boolean));

  rssResults.forEach((result, i) => {
    if (result.status === "fulfilled") {
      // Prefer explicit Inbox saves over the same URL from RSS.
      allItems.push(...result.value.filter((item) => !manualLinkSet.has(item.link)));
    } else {
  ...
  if (vicResult.error || vicResult.warning) {
    failed.push("Value Investors Club");
  }

  const fundLetterItems = fundLettersResult.items || [];
  if (fundLetterItems.length) {
    allItems.push(...fundLetterItems);
  }
  if (fundLettersResult.error) {
    failed.push("Fund Letters");
  } else if (Array.isArray(fundLettersResult.errors) && fundLettersResult.errors.length) {
    for (const entry of fundLettersResult.errors) {
      failed.push(entry.fund || "Fund Letters");
    }
  }

  if (manualResult.error) failed.push("Saved Links");

  const writings = writingsResult.items || [];
  if (writingsResult.error) failed.push("Writing");

  allItems.push(...bookmarks);
  allItems.push(...manualLinks);
  allItems.push(...writings);

  const fundLetterLinks = new Set(fundLetterItems.map((item) => item.link));
  const isDynamicItem = (item) =>
    ...
    manualLinkSet.has(item.link) ||
    fundLetterLinks.has(item.link);

  const dynamicItems = allItems.filter(isDynamicItem);
  const otherItems = allItems.filter((item) => !isDynamicItem(item));
  ...
  return {
    ...
    meta: {
      ...
      fundLetters: fundLettersResult.items?.length || 0,
