...
async function buildFeedResponse(sources) {
  ...
  const [rssResults, spotifyResult, bookmarks, manualResult] = await Promise.all([
  ...
  const dynamicItems = allItems.filter(
    (item) => item.category === "Spotify" || item.category === "Bookmarks" || item.source === "Saved Links"
  );
  const otherItems = allItems.filter(
    (item) => item.category !== "Spotify" && item.category !== "Bookmarks"
  );
