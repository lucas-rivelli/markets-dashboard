...
/** Only surface ideas from the newest N calendar days (member "today"). */
const VIC_FEED_DAYS = Math.max(1, Number(process.env.VIC_FEED_DAYS || 1) || 1);
...
/** Keep only ideas from the newest VIC_FEED_DAYS calendar days. */
function keepNewestCalendarDays(items, days = VIC_FEED_DAYS) {
  const list = Array.isArray(items) ? items : [];
  if (!list.length || days <= 0) return [];
  ...
  const newestDays = [...daySet].sort((a, b) => (a < b ? 1 : a > b ? -1 : 0)).slice(0, days);
  const allowed = new Set(newestDays);
  return list
    .filter((item) => allowed.has(calendarDay(item?.date)))
...
async function fetchVicIdeas({ force = false } = {}) {
  ...
  if (cacheFresh) {
    return { items: keepNewestCalendarDays(cached), cached: true };
  }
  ...
  async function adopt(items, meta = {}) {
    ...
    if (cached?.length && newestDate(items) < newestDate(cached)) {
      return {
        items: keepNewestCalendarDays(cached),
        cached: true,
        ...meta,
        skipped: "guest older than cache",
      };
    }
    const feedItems = keepNewestCalendarDays(items);
    state.items = feedItems;
    state.fetchedAt = now;
    await persistCache();
    return { items: feedItems, cached: false, ...meta };
  }
  ...
  try {
    ...
    const htmlChallenge = err?.code === "vic_html_challenge";
    ...
    if (cookie && sessionRejected) {
      try {
        ...
        if (cached?.length) {
          return { items: keepNewestCalendarDays(cached), cached: true, error: err.message };
        }
        throw guestErr;
      }
    }
