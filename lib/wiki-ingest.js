...
const { maybeEnhanceWithLlm } = require("./wiki-llm");
...
async function ingestWikiItem(idOrRecord, { dryRun = false, useLlm = true } = {}) {
  const record =
  ...
  const llm = useLlm ? await maybeEnhanceWithLlm(record) : null;
  const source = buildSourcePage(record, llm);
...
async function ingestAllWiki({ dryRun = false, useLlm = true } = {}) {
  const ids = listInboxIds();
  const results = [];
  for (const id of ids) {
    results.push(await ingestWikiItem(id, { dryRun, useLlm }));
  }
