...
const KB_WIKI_SOURCES_DIR = path.join(KB_DIR, "wiki", "sources");
...
function savedRecord(filePath) {
  ...
  return {
    ...
    folders: normalizeArray(item.folders),
    tags: normalizeArray(item.tags),
    summary: "",
    snippet: compact(item.snippet),
    content_kind: compact(item.content_kind),
    highlight_count: Array.isArray(item.highlights) ? item.highlights.length : 0,
    content_text: compact(item.content_text),
    content_meta: item.content_meta && typeof item.content_meta === "object" ? item.content_meta : null,
    inbox_path: toRelative(filePath),
    note_path: null,
  };
}

function wikiSourceRecord(filePath) {
  ...
  return {
    ...
    type: compact(frontmatter.type || "source"),
    ...
    folders: normalizeArray(frontmatter.folders),
    tags: normalizeArray(frontmatter.tags),
    summary: compact(frontmatter.summary) || excerpt(body),
    snippet: "",
    content_kind: compact(frontmatter.content_kind),
    highlight_count: 0,
    content_text: excerpt(body, 200000),
    content_meta: null,
    inbox_path: frontmatter.inbox_path || null,
...
function mergeRecord(base, overlay) {
  return {
    ...
    folders: overlay.folders?.length ? overlay.folders : base.folders || [],
    tags: overlay.tags?.length ? overlay.tags : base.tags || [],
    snippet: overlay.snippet || base.snippet || "",
    summary: overlay.summary || base.summary || "",
    highlight_count: overlay.highlight_count ?? base.highlight_count ?? 0,
    content_kind: overlay.content_kind || base.content_kind || "",
    content_text: overlay.content_text || base.content_text || "",
    content_meta: overlay.content_meta || base.content_meta || null,
...
function buildKbIndex() {
  ...
  const wikiSourceFiles = listFiles(KB_WIKI_SOURCES_DIR, ".md");
  ...
  for (const filePath of wikiSourceFiles) {
    const record = wikiSourceRecord(filePath);
