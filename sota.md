...
**Features:** merged Latest timeline · Sources launchpad grouped by category · email-style item states (`Inbox`, `Trash`) plus folders/tags · search by title/source · morning cron + first-visit-of-day live fetch · manual refresh (`?fresh=1`) · 30-min edge cache.
...
│ Trash    │ snippet rows,  │ Sources launchpad;│ (SVG graph,  │
...
- **Folders + tags** sync across devices via `GET/PUT /api/workspace` → `data/workspace.json` (GitHub in production). localStorage is a fast cache; the remote workspace is the cross-device source of truth. **Folder ❧** and **Tag** assign items. Folders are email-style hierarchical paths (`Parent/Child`); creating a subfolder auto-creates ancestors, and renaming/moving/excluding a parent updates descendant paths plus item assignments. The old Save UI is intentionally removed as redundant with mailbox states.
